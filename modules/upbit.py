...
# Keys
access_key = 'wSSGbYVgZu5RDdxX5DZsDGtyA8qFZtqLGjSxxcJ3'
secret_key = 'PTtN4phEo7JpQycW0yTI3cFEU3XWmNS0XnaiCD8P'
...
#   1) 전체 리스트 : 리스트
# -----------------------------------------------------------------------------
def get_items(market, except_item = None):
    try:
...
# RSI 계산
def rsi_1(ohlc: pd.DataFrame, period: int = 14):
    
    
    ohlc["close"] = ohlc["close"]
    ohlc['change'] = ohlc["close"] - ohlc['close'].shift(1) 
    
    
    ohlc['up'] = np.where(ohlc['change']>=0, ohlc['change'], 0)
    ohlc['down'] = np.where(ohlc['change'] <0, ohlc['change'].abs(), 0)
    
    ohlc['AU'] = ohlc['up'].ewm(alpha=1/period, min_periods=period).mean()
    ohlc['AD'] = ohlc['down'].abs().ewm(alpha=1/period, min_periods=period).mean()
    
    ohlc['RSI'] = ohlc['AU'] / (ohlc['AU'] + ohlc['AD']) * 100
    return ohlc['RSI']
...
#   1) RSI 값
# -----------------------------------------------------------------------------
def get_candle_rsi(candle_datas):
    try:
        ...
        # # 캔들 데이터만큼 수행
        # for candle_data_for in candle_datas:
        #     df = pd.DataFrame(candle_data_for, index=[0])
